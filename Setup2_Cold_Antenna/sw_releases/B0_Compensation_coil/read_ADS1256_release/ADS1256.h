...
public:
  ...
  void setChannel(unsigned char AIN_P, unsigned char AIN_N = ADS1256_MUX_AINCOM);
  void begin(unsigned char drate, unsigned char gain, bool bufferenable, bool clkout_disable=0);
  void begin();
  uint8_t getStatus();
  void waitDRDY();
  boolean isDRDY();
  ...
  //*** added *** 
  float readCurrentChannelC();    //in continuous mode, avoid 7us delay
  long readCurrentChannelRawC();  //in continuous mode, avoid 7us delay
  void startRdatac ();  
